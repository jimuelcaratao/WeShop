    ...
    <div class="mt-3 flex flex-col justify-start">
        <div class="flex flex-row items-center">
            ...
            @if (Laravel\Jetstream\Jetstream::managesProfilePhotos())
                <img class="h-12 w-12 rounded-full object-cover mr-3 border p-1"
                    src="{{ $reviews->user->profile_photo_url }}" alt="{{ $reviews->user->name }}" />
            @else
                <span class="inline-flex rounded-md">
                    {{ $reviews->user->name }}
                    <svg class="ml-2 -mr-0.5 h-4 w-4 mr-3 border p-1" xmlns="http://www.w3.org/2000/svg"
                ...
                <div class="flex flex-row">
                    ...
                    @if (!empty($reviews->user->email_verified_at))
                        <div class=" ml-2 text-green-600">
                            <svg xmlns="http://www.w3.org/2000/svg" class="verified h-5 w-5" viewBox="0 0 20 20"
                                fill="currentColor">
                                <path fill-rule="evenodd"
                    ...
                    @if (Auth::user()->is_admin == 1)
                        <form action="{{ route('delete_review', [$reviews->id]) }}" method="POST">
                            @method('DELETE')
                            @csrf
                            <button type="submit" class=" ml-2 text-red-600">
