<x-normal_user>
    ...
    <div class="bg-white p-5 my-12 w-11/12 mx-auto flex flex-col justify-center items-center">
        ...
        <div class="p-5 w-4/5 border-b border-gray-400">
            <div class="grid grid-cols-3 gap-4">
                <div class="col-span-3 md:col-span-2 z-10">
                    <!-- Swiper -->
                    <div style="--swiper-navigation-color: #fff; --swiper-pagination-color: #fff"
                        ...
                        <div class="swiper-wrapper">
                            ...
                            @if (!empty($product->product_photos->photo_1))
                                <div class="swiper-slide">
                                    <img id="myImg_1" class="product_image"
                                        src="{{ asset('storage/media/products/' . $product->product_code . '_photo_1_' . $product->product_photos->photo_1) }}" />
                                </div>
                            @endif
                            @if (!empty($product->product_photos->photo_2))
                                <div class="swiper-slide">
                                    <img id="myImg_2" class="product_image"
                                        src="{{ asset('storage/media/products/' . $product->product_code . '_photo_2_' . $product->product_photos->photo_2) }}" />
                                </div>
                            @endif
                            @if (!empty($product->product_photos->photo_3))
                                <div class="swiper-slide">
                                    <img id="myImg_3" class="product_image"
                                        src="{{ asset('storage/media/products/' . $product->product_code . '_photo_3_' . $product->product_photos->photo_3) }}" />
                                </div>
                            @endif
                ...
                <div class="flex flex-col z-20 bg-white pl-20">
    ...
    5 - round($product_ave_reviews, 0),
                    ...
                    @if (!empty($product->product_price->discounted_price))
                        <h1 class="mt-5 text-2xl md:text-4xl font-bold text-yellow-600">&#8369;
                            @convert($product->product_price->discounted_price)</h1>
                        <h6 class="mt-5 text-sm md:text-md font-bold text-red-600">&#8369; <span
                    ...
                    @else
                        ...
                        <h1 class="mt-5 text-2xl md:text-4xl font-bold text-yellow-600">&#8369;
                            @convert(optional($product->product_price)->price)</h1>
                    @endif
        ...
        <div class="p-5 w-4/5 border-b border-gray-400">
            ...
            <div class="mt-3 flex flex-col justify-start">
                ...
                @endempty
                <textarea disabled id="specs" name="specs" rows="3"
                    class="resize-none mt-1 block w-full sm:text-sm border-none rounded-md">{{ $product->description }}</textarea>
                <br>
                <h1 class="mb-5 font-bold">Product's specification</h1>
                @empty($product->description)
                    No specification provided
                @endempty
                <textarea textarea disabled id="specs" name="specs" rows="3"
                    class="resize-none mt-1 block w-full sm:text-sm border-none rounded-md">{{ $product->specs }}</textarea>
            </div>
        </div>
