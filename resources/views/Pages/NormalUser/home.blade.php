<x-normal_user>
    <div class="flex flex-col space-y-20">
        ...
        <div class="glider-contain h-1/2 w-screen">
            ...
            <div class="flex justify-between items-center">
                <x-jet-button aria-label="Previous" class="glider-prev">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-10 w-10 fill-current" fill="none"
            ...
            <div class="relative glider-contain relative h-1/2 w-screen shadow-md rounded-lg">
                ...
                <div class="products bg-white flex justify-evenly items-center">
                    @forelse ($products as $product)
                        <div
                            class="h-full w-60 p-5 flex flex-col justify-center items-center space-y-5 hover:shadow-md bg-gray-50">
                            <img src="{{ asset('storage/media/products/main_' . $product->product_code . '_' . $product->default_photo) }}"
                                alt="{{ $product->product_name }}" class="block h-2/4 w-auto mx-auto">
                            <p class="text-sm sm:text-md font-semibold">{{ $product->product_name }}</p>
                            @if (!empty($product->product_price->discounted_price))
                                <h1 class="mr-2 0">&#8369; @convert($product->product_price->discounted_price)
                                </h1>
                                <p class="text-red-600 text-sm">&#8369; <span
                                        class="line-through ">@convert($product->product_price->price)</span>
                                    <span>
                            ...
                            @else
                                ...
                                <h1 class=" ">&#8369; @convert(optional($product->product_price)->price)
                                </h1>
                            @endif
                            <x-jet-secondary-button>
                                <a href="/product/{{ $product->product_code }}"
                                    class="flex flex-row justify-center items-center">View product
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 ml-3" viewBox="0 0 20 20"
                                        fill="currentColor">
                    ...
                    @empty
                        <div class="flex flex-col justify-center items-center p-5 mx-auto">
                            ...
                            <h1 class="text-center font-bold text-md sm:text-lg mt-5">No products found in the database.
                            </h1>
                        </div>
                    @endforelse
                </div>
                ...
                <div class="flex justify-between items-center">
                    <x-jet-button aria-label="Previous" class="absolute top-1/2 left-0 glider-prev-products">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 fill-current" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
                        </svg>
                    </x-jet-button>
        ...
        <div class="h-auto md:h-screen w-screen flex flex-col md:flex-row bg-gray-900">
            <div>
                <a href="{{ route('catalog.collection', ['components', 'power supply']) }}">
                    <img src="{{ asset('images/home-squares/Network.jpg') }}" class="h-1/2 w-full block mx-auto"
                        alt="A network">
                </a>
                <a href="{{ route('catalog.collection', ['peripherals', 'headset']) }}">
                    <img src="{{ asset('images/home-squares/Headset.jpg') }}" class="h-1/2 w-full block mx-auto"
                        alt="A headset">
                </a>
            </div>
            <div>
                <a href="{{ route('catalog.collection', ['mobile devices', 'laptops']) }}">
                    <img src="{{ asset('images/home-squares/Laptop.jpg') }}" class="h-1/2 w-full block mx-auto"
                        alt="A laptop">
                </a>
                <a href="{{ route('catalog.collection', ['peripherals', 'monitor']) }}">
                    <img src="{{ asset('images/home-squares/Monitor.jpg') }}" class="h-1/2 w-full block mx-auto"
                        alt="A monitor">
                </a>
            </div>
        </div>
        ...
        <div class="px-6 sm:px-12">
            ...
            <div class="relative glider-contain relative h-1/2 w-screen shadow-md rounded-lg">
                ...
                <div class="most-viewed bg-white flex justify-evenly items-center">
                    ...
                    @empty
                        <div class="flex flex-col justify-center items-center p-5 mx-auto">
                            <img src="{{ asset('images/undraw_empty_cart_co35.svg') }}"
                                alt="A boy with a empty cart">
                            <h1 class="text-center font-bold text-md sm:text-lg mt-5">No products found in the database.
                            </h1>
                        </div>
                    @endforelse
        ...
        <div class="p-5 h-1/2 w-screen bg-secondary flex flex-col md:flex-row justify-center items-center">
            <img src="{{ asset('images/undraw_community_8nwl.svg') }}" class="h-1/2 w-1/2 block mx-auto"
                alt="Three girls facing forward...">
            <div class="flex flex-col justify-center items-center p-5">
                <h1 class="text-3xl md:text-5xl text-gray-100">Easy shopping for computer enthusiastics</h1>
                <br>
                <p class="text-gray-200">There are many products here. Go explore this website, you might find what you
                    like...</p>
            </div>
        </div>
    </div>
